...
bool UpdateClass::begin(size_t size, int command, int ledPin, uint8_t ledOn) {
    ...
    if (command == U_FLASH) {
        Serial.println("command == U_FLASH");
        ...
        /*
        Serial.print("OTA Partition:");Serial.println( _partition->label);
        Serial.print("OTA address:");Serial.println( _partition->address);
        Serial.print("OTA size:");Serial.println( _partition->size);
        Serial.print("OTA type:");Serial.println( _partition->type);
        Serial.print("OTA subtype:");Serial.println( _partition->subtype);
        */
    ...
    //Serial.print("partition size: ");Serial.println(_partition->size);
...
bool UpdateClass::_writeBuffer(){
    ...
    if(!_progress && _command == U_FLASH){
        ...
        if(_buffer[0] != ESP_IMAGE_HEADER_MAGIC){
            Serial.println("_writeBuffer() _buffer[0] != ESP_IMAGE_HEADER_MAGIC");
...
bool UpdateClass::_verifyHeader(uint8_t data) {
    if(_command == U_FLASH) {
        if(data != ESP_IMAGE_HEADER_MAGIC) {
            ...
            Serial.print("ESP_IMAGE_HEADER_MAGIC: ");Serial.println(ESP_IMAGE_HEADER_MAGIC);
            _abort(UPDATE_ERROR_MAGIC_BYTE);
            return false;
        }
        return true;
...
size_t UpdateClass::writeStream(Stream &data) {
    ...
    size_t toRead = 0;
    if(hasError() || !isRunning()){
        Serial.println("has error or isn't running");
        return 0;
    }

    if(!_verifyHeader(data.peek())) {
        _reset();
        Serial.println("_verifyHeader peak failed");
        return 0;
    }
    ...
    while(remaining()) {
        ...
        if(toRead == 0) { //Timeout
            delay(100);
            Serial.println("toRead = 0 As");
            toRead = data.readBytes(_buffer + _bufferLen, bytesToRead);
            if(toRead == 0) { //Timeout
                Serial.println("toRead = 0 B");
        ...
        if((_bufferLen == remaining() || _bufferLen == SPI_FLASH_SEC_SIZE) && !_writeBuffer()){
            Serial.println("_bufferLen == remaining() || _bufferLen == SPI_FLASH_SEC_SIZE) && !_writeBuffer()");
            return written;
        }
        written += toRead;
        //Serial.println("written: "+written);
    }
    Serial.println("writeStream end");
    return written;
}
